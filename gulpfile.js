...
gulp.task('toES5',function(){
  console.log(cptsJSFile);
  return gulp.src(cptsJSFile)
  .pipe(plumber({errorHandler: notify.onError('Error:<%= error.message %>;')}))
  .pipe(babel({
    presets: [
      [ "es2015", { "modules": false } ]
...
const cptsBaseFiles = getSrc(
  ...
  'Icon, Button, Gallery, Message, Pagination, Dropdown, Tabs', // ygf
  // 'Gallery, Pagination, Dropdown, Tabs', // ygf-mobile
  'js',
  'components/js/'
);
